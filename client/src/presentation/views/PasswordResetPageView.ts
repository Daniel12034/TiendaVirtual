import { PasswordResetPageState } from "../types.js";
import { escapeHtml, formatDateTime } from "../utils/formatters.js";

export class PasswordResetPageView {
  constructor(private readonly root: HTMLElement) {}

  public render(state: PasswordResetPageState): void {
    const subtitle =
      state.status === "valid"
        ? `
          <p class="auth-page__support">
            Vas a actualizar la contrasena de <strong>${escapeHtml(
              state.email ?? ""
            )}</strong>.
            ${
              state.expiresAt
                ? ` Este enlace vence el ${escapeHtml(
