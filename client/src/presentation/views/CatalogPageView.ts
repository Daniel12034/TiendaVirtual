import { CatalogPageState } from "../types.js";
...
export class CatalogPageView {
  ...
  public render(state: CatalogPageState): void {
    const categoryMarkup = `
      <div class="category-filter" aria-label="Categorias disponibles">
        ...
        >
          <span>Todas</span>
          <small>${state.categories.reduce(
            (accumulator, category) => accumulator + category.cantidadProductos,
            0
          )}</small>
        </button>
