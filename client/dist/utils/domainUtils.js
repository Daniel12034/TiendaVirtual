...
export function validarEnteroPositivo(valor, campo) {
    if (!Number.isInteger(valor) || valor <= 0) {
        throw new Error(`${campo} debe ser un entero positivo.`);
    }
}
export function validarEnteroNoNegativo(valor, campo) {
    if (!Number.isInteger(valor) || valor < 0) {
        throw new Error(`${campo} debe ser un entero no negativo.`);
    }
}
export function validarDecimalNoNegativo(valor, campo) {
    if (Number.isNaN(valor) || valor < 0) {
        throw new Error(`${campo} debe ser un decimal no negativo.`);
    }
}
export function agregarMinutos(fecha, minutos) {
